...
class Wave1dSolver{
    ...
    void solve() //F,Gの時間ステップ
    {
        for(int i=0;i<nx;i++)
        {
            dF[i]=(F_old[i+1]-F_old[i-1])/(2*dx);
            dG[i]=(G_old[i+1]-G_old[i-1])/(2*dx);
        }
        for(int i=0;i<nx;i++)
        {
            F[i]=(D*((1-0.25*dt*(nu-mu))*((1-k)*F_old[i-j]-0.5*(1-k)*k*dx*dF[i-j]+k*F_old[i-j-1]+0.5*k*(1-k)*dx*dF[i-j-1])+0.25*dt*(nu+mu)*((1-k)*G_old[i-j]-0.5*(1-k)*k*dx*dG[i-j]+k*G_old[i-j-1]+0.5*k*(1-k)*dx*dG[i-j-1]))
                +B*((1-0.25*dt*(nu+mu))*((1-k)*G_old[i+j]+0.5*(1-k)*k*dx*dG[i+j]+k*G_old[i+j+1]-0.5*k*(1-k)*dx*dG[i+j+1])+0.25*dt*(nu-mu)*((1-k)*F_old[i+j]+0.5*(1-k)*k*dx*dF[i+j]+k*F_old[i+j+1]-0.5*k*(1-k)*dx*dF[i+j+1])))/(A*D-B*C);
                
            G[i]=(A*((1-0.25*dt*(nu+mu))*((1-k)*G_old[i+j]+0.5*(1-k)*k*dx*dG[i+j]+k*G_old[i+j+1]-0.5*k*(1-k)*dx*dG[i+j+1])+0.25*dt*(nu-mu)*((1-k)*F_old[i+j]+0.5*(1-k)*k*dx*dF[i+j]+k*F_old[i+j+1]-0.5*k*(1-k)*dx*dF[i+j+1]))
                +C*((1-0.25*dt*(nu-mu))*((1-k)*F_old[i-j]-0.5*(1-k)*k*dx*dF[i-j]+k*F_old[i-j-1]+0.5*k*(1-k)*dx*dF[i-j-1])+0.25*dt*(nu+mu)*((1-k)*G_old[i-j]-0.5*(1-k)*k*dx*dG[i-j]+k*G_old[i-j-1]+0.5*k*(1-k)*dx*dG[i-j-1])))/(A*D-B*C);
        }
        for(int i=0;i<nx;i++)
        {
            n[i]=0.5*(F[i]+G[i]);
